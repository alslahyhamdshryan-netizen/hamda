...
export const dashboardApi = { get: () => api<any>("/api/dashboard/"), dailyReport: (date?: string) => api<any>(`/api/reports/daily/${date ? `?date=${date}` : ""}`), createEmployee: (data: object) => api<{ ok: boolean; id: number }>("/api/employees/", { method: "POST", body: JSON.stringify(data) }), createAttendance: (data: object) => api<{ ok: boolean }>("/api/attendance/", { method: "POST", body: JSON.stringify(data) }), createLeave: (data: object) => api<{ ok: boolean }>("/api/leaves/", { method: "POST", body: JSON.stringify(data) }) };
